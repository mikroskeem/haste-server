...
RedisDocumentStore.connect = function(options) {
    ...
    RedisDocumentStore.client.select(index, (err, reply) => {
        if (err) {
            winston.error(
...
RedisDocumentStore.prototype.set = function(key, data, callback, skipExpire) {
    var _this = this;
    RedisDocumentStore.client.set(key, data, (err, reply) => {
        if (err) {
            callback(false);
        }
        else {
...
RedisDocumentStore.prototype.setExpiration = function(key) {
    if (this.expire) {
        RedisDocumentStore.client.expire(key, this.expire, (err, reply) => {
            if (err) {
