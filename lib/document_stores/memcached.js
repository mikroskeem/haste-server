...
const MemcachedDocumentStore = function(options) {
...
MemcachedDocumentStore.connect = function(options) {
    ...
    this.client.connect();
    this.client.on('connect', () => winston.info(`connected to memcached on ${host}:${port}`));
    this.client.on('error', (e) => winston.info('error connecting to memcached', {error: e});
};

// Save file in a key
MemcachedDocumentStore.prototype.set = function(key, data, callback, skipExpire) {
    MemcachedDocumentStore.client.set(key, data, (err, reply) => {
...
MemcachedDocumentStore.prototype.get = function(key, callback, skipExpire) {
    var _this = this;
    MemcachedDocumentStore.client.get(key, (err, reply) => {
